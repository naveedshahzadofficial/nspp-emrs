            ...
            <div class="card-body">
                ...
                <div class="table-responsive ">
                    ...
                    >
                        ...
                        <tbody>
                        <template v-for="diseaseType in diseaseTypes.data" :key="diseaseType.id">
                            <tr>
                                <td>{{ diseaseType.type_name }}</td>
                                <td class="text-center">
                                    <span class="badge font-weight-bold"
                                          :class="[diseaseType.status?'badge-success':'badge-danger']">
                                        {{ diseaseType.status?'Active':'Inactive' }}
                                    </span>
                                </td>
                                <td class="text-center">
                                    <Link
                                        v-if="hasPermission('view disease types')"
                                        :href="route('disease-types.show', diseaseType.id)"
                                    ...
                                    <Link
                                        v-if="hasPermission('update disease types')"
                                        :href="route('disease-types.edit', diseaseType.id)"
                                    ...
                                    <a
                                        v-if="hasPermission('toggle status disease types')"
                                        @click.prevent="toggleStatus('disease-types.change-status', diseaseType)"
                                       class="btn btn-icon btn-circle btn-sm me-2"
                                       :class="[diseaseType.status?'btn-danger':'btn-success']"
                                       data-bs-toggle="tooltip"
                                       data-bs-placement="top"
                                       :title="[diseaseType.status?'Deactivate':'Activate']">
                                        <i class="icon-xl fas"
                                           :class="[diseaseType.status?'fa-toggle-off':'fa-toggle-on']"
                                        ></i>
                                    </a>

                                    <a
                                        v-if="hasPermission('delete disease types')"
                                        @click.prevent="destroy('disease-types.destroy', diseaseType.id)"
                ...
                <Pagination :meta="diseaseTypes?.meta" :links="diseaseTypes?.links" />
            </div>
            </div>
...
const props = defineProps({
    diseaseTypes: { type: Object, required: true },
    filters: Object
});
